...
const props = defineProps({
  ...
  onTilesLoaded: { type: Function, required: false },
});
...
const infiniteGridContainer = ref(null);
let infiniteGridInstance = null;
...
onMounted(async () => {
  if (infiniteGridContainer.value) {
    ...
    infiniteGridContainer.value.addEventListener(
      "tileClicked",
      handleTileClicked,
    );
  }
...
onBeforeUnmount(() => {
  if (infiniteGridInstance) {
    if (infiniteGridContainer.value) {
      infiniteGridContainer.value.removeEventListener(
        "tileClicked",
        handleTileClicked,
      );
    }
