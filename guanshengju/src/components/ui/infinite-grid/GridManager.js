...
export class GridManager {
    ...
    for (let r = -1; r <= 1; r++) {
      for (let c = -1; c <= 1; c++) {
        this.host.tileGroupsData.push({
          basePos: new Vec3(
            this.host.GRID_WIDTH * c,
            this.host.GRID_HEIGHT * r,
            0,
          ),
          offset: { x: 0, y: 0 },
        });
  ...
  createBackgroundMesh(
    ...
    const backgroundProgram = this.createBackgroundProgram(
      groupIndex,
      tileIndex,
    );
  ...
  createForegroundMesh(
    ...
    const foregroundProgram = this.createForegroundProgram(
      groupIndex,
      tileIndex,
    );
   ...
   */
  getTileKey(groupIndex, tileIndex) {
   ...
   */
  getCardTextureIndex(groupIndex, tileIndex) {
   ...
   */
  getCardForegroundTexture(groupIndex, tileIndex) {
   ...
   */
  getCardBackgroundTexture(groupIndex, tileIndex) {
    if (this.host.cardTextures.length === 0) return null;
    const textureIndex = this.getCardTextureIndex(groupIndex, tileIndex);
   ...
   */
  createBackgroundProgram(groupIndex, tileIndex) {
    ...
    this.host.staticUniforms.set(
      this.getTileKey(groupIndex, tileIndex),
      uniforms,
    );
   ...
   */
  createForegroundProgram(groupIndex, tileIndex) {
    if (!this.host.renderer) throw new Error("Renderer not initialized");
   ...
   */
  async generateTexturesForCardData() {
    if (!this.host.renderer) throw new Error("Renderer not initialized");
    ...
    const texturePromises = this.host.cardData.map(async (card) => {
      const foreground = await generateForegroundTexture(
        card,
        this.host.renderer,
      );
      const background = await generateBackgroundTexture(
        card,
        this.host.renderer,
      );
   ...
   */
  getCardDataForTile(groupIndex, tileIndex) {
   ...
   */
  async updateCardData(newCardData) {
      ...
      for (let row = 0; row < this.host.GRID_ROWS; row++) {
        for (let col = 0; col < this.host.GRID_COLS; col++) {
          ...
          if (foregroundMesh && foregroundMesh.program) {
            const newForegroundTexture = this.getCardForegroundTexture(
              groupIndex,
              tileIndex,
            );
          ...
          if (backgroundMesh && backgroundMesh.program) {
            const newBackgroundTexture = this.getCardBackgroundTexture(
              groupIndex,
              tileIndex,
            );
