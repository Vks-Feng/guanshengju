...
export class EventHandler {
  ...
  onPointerMove(e) {
    ...
    const distanceX =
      (this.host.startPosition.x - clientX) * this.host.scroll.scale;
    const distanceY =
      (this.host.startPosition.y - clientY) * this.host.scroll.scale;
  ...
  onPointerUp(e) {
    ...
    if (this.host.currentHoveredTileKey) {
      const mesh = this.host.backgroundMeshMap.get(
        this.host.currentHoveredTileKey,
      );
      if (mesh) {
  ...
  performTileClick() {
    ...
    if (hits.length > 0) {
      ...
      if (userData) {
        ...
        const cardData = this.host.getCardDataForTile(
          userData.groupIndex,
          userData.tileIndex,
        );
  ...
  addEventListeners() {
    ...
    this.host.container.addEventListener("touchstart", this.onPointerDown, {
      passive: false,
    });
    this.host.container.addEventListener("touchmove", this.onPointerMove, {
      passive: false,
    });
    this.host.container.addEventListener("touchend", this.handleTouchEnd, {
      passive: true,
    });
