...
<template>
  <div class="relative block" :class="[props.class]">
    <div class="relative grid w-full max-w-full items-center justify-center" :class="[props.cards.length < 4 ? `grid-cols-${props.cards.length}` : 'grid-cols-4']" :style="{
  transform: `perspective(${perspective}px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`,
}">
      <div v-for="(item, index) in props.cards" :key="index" ref="card" class="card group block rounded-xl border border-transparent px-6 py-6 transition-all duration-200 min-w-[100px] min-height-[100px] flex items-center justify-center" :style="{
  zIndex: index + 1,
}">
